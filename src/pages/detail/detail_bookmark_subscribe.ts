...
const res = await axios.get<ApiUserDetailRes>(`/users/${loginUserId}`);
const mydataitem = res.data.item;
console.log(mydataitem, '=================');

const mybookmark = mydataitem.bookmarkedBy; // 배열
const mybookmarkuser = mybookmark.userItems;
const target_Id = Object(mybookmarkuser)[0].target_id;
console.log('target_Id!!!!!!!!!', mybookmark, target_Id);
...
async function checkLike() {
  try {
    const res = await axios.get<BookmarkResponse>(
      `/bookmarks/post/${loginUserId}`,
    );
...
const bookmark_res = await axios.get<BookmarkResponse2>(`/bookmarks/user/`);
